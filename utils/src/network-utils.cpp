...
NetworkUtils::BecomeDaemon( string log_filename )
{
    ...
    // call setsid).
    int pid = fork();
    if( pid < 0 ) t.FatalError( "First fork() failed." );
    if( pid > 0 ) exit(0);

    if( setsid() == -1 ) t.FatalError( "setsid() failed." );
    pid = fork();
    if( pid < 0 ) t.FatalError( "Second fork() failed." );    
    if( pid > 0 ) exit(0);
    ...
    if( chdir( "/" ) == -1 ) t.FatalError( "chdir() failed." );
...
NetworkUtils::ForkGrandchild( int socket )
{
    ...
    int pid = fork();
    if( pid < 0 ) t.FatalError( "fork() failed" );
    if( pid > 0 ) // parent
    {
        close( socket );
    ...
    int pid2 = fork();
    if( pid2 < 0 ) t.FatalError( "fork() failed" );
    if( pid2 > 0 ) exit(0);
