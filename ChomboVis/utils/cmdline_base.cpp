...
{ 
    m_argmap["debug_level"] = new SetInt(&s_debug_level, "");
}


CmdlineBase::~CmdlineBase()
{
    for( map<string,SetType*>::iterator iter = m_argmap.begin();
         iter != m_argmap.end();
         ++iter )
    {
        delete iter->second;
 ...
 */
void
CmdlineBase::CheckForCmdlineFile( string filename )
{
...
void CmdlineBase::ParseCommandLine( int argc, char *argv[] )
{
    ...
    {
        for( map<string,SetType*>::iterator i=m_argmap.begin();
                                            i!=m_argmap.end();
                                            i++ )
        {
...
*/
string CmdlineBase::ArgName( string str )
{
    char* buf = new char[ str.length()+1 ];
    strcpy( buf, str.c_str() );
    string result( strtok( buf, "=" ) );
    delete[] buf;
    return result;
}
...
*/
string CmdlineBase::ArgVal( string str )
{
    string result;
    char * buf = new char[ str.length()+1 ];
    strcpy( buf, str.c_str() );
...
{
    Trace t("CmdlineBase::");
    for( map<std::string, SetType*>::const_iterator iter = m_argmap.begin();
         iter != m_argmap.end();
         ++iter )
    {
...
int CmdlineBase::s_debug_level = 3;
